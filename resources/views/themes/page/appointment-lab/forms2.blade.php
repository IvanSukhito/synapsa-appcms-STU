        ...
        <div class="container-fluid">
            <div class="card {!! $printCard !!}">
                ...
                @if(in_array($viewType, ['create']))
                    {{ Form::open(['route' => ['admin.' . $thisRoute . '.storeHasilLab', $data->{$masterId}], 'files' => true, 'id'=>'form', 'role' => 'form'])  }}
                @elseif(in_array($viewType, ['edit']))
                    {{ Form::open(['route' => ['admin.' . $thisRoute . '.storeHasilLab', $data->{$masterId}], 'method' => 'POST', 'files' => true, 'id'=>'form', 'role' => 'form'])  }}
                @else
                ...
                <div class="card-body">
                    <div class="form-group">
                        <label for="import_product">{{ __('general.upload_hasil_lab') }} <span class="text-red">*</span></label>
                        {{ Form::file('form_patient', ['class' => $errors->has('form_patient') ? 'form-control dropify is-invalid' : 'form-control dropify', 'id' => 'form_patient', 'required' => true, 'autocomplete' => 'off', 'accept' => '.pdf, .doc']) }}
                    </div>
                </div>
                <!-- /.card-body -->
