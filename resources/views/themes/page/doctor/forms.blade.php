        ...
        <div class="container-fluid">
            <div class="card {!! $printCard !!}">
                ...
                <div class="card-body">
                    ...
                    @include(env('ADMIN_TEMPLATE').'.page.doctor.generate_forms2')
...
@section('script-bottom')
    ...
    <script>
       $(document).ready(function() {
            $('#total_service').change();
            var elements = document.getElementById("#service_id").elements;
        });

        $('#total_service').change(function() {
            let totalService = 0;
            if ($(this).val() !== '') {
                totalService = parseInt($(this).val());
            }
            ...
            for (let i=0; i<totalService; i++) {
                html += '<div class="form-group">' +
                    ...
                    '<option type="select2" selected="selected">'+'</option>'+
                    '</select>'+
                    '<br>'+
                    '</div>'+
                    '<div class="form-group">' +
                    '<label for="price">{{ __('general.price') }} ' + (i + 1) + ' <span class="text-red">*</span></label>' +
                    '<input type="text" id="price_' + i +'" name="price[]" class="form-control" placeholder="price"> ' +
                    '<br>'+
                    '</div>';
            }
